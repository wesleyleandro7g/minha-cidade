-- Seed demo data wrapper
BEGIN;
CREATE TEMP TABLE IF NOT EXISTS _seed_sql_chunks (part int PRIMARY KEY, content text NOT NULL);
TRUNCATE _seed_sql_chunks;
INSERT INTO _seed_sql_chunks (part, content) VALUES (1, '-- Continue seed (slug-based mapping via temp table rebuilt from existing rows)
BEGIN;
...
INSERT INTO busine');
INSERT INTO _seed_sql_chunks (part, content) VALUES (2, 'ss_hours (tenant_id, business_id, weekday, opens_at, closes_at, closed)
...
JOIN _seed_ids tt ON tt.fixture_id = ''t-salinas'' AND tt.entity_type = ''tenant''
WHERE ');
INSERT INTO _seed_sql_chunks (part, content) VALUES (3, 'tb.fixture_id = ''b-studio-bella'' AND tb.entity_type = ''business''
AND NOT EXISTS (
  SELECT 1 FROM business_hours bh
  JOIN _seed_ids tb2 ON tb2.db_id = bh.business_id AND tb2.fixture_id = ''b-studio-bella''
  WHERE bh.weekday = 6
...
ON CONFLICT (tenant_id, slug) DO NOTHING;
INSERT INTO');
INSERT INTO _seed_sql_chunks (part, content) VALUES (4, ' _seed_ids (fixture_id, entity_type, db_id)
...
FROM _seed_i');
INSERT INTO _seed_sql_chunks (part, content) VALUES (5, 'ds tb
JOIN _seed_ids tt ON tt.fixture_id = ''t-montes-claros'' AND tt.entity_type = ''tenant''
WHERE tb.fixture_id = ''b-mc-bistro'' AND tb.entity_type = ''business''
AND NOT EXISTS (
  SELECT 1 FROM business_hours bh
...
SELECT tt.db_id, tb.db_id,
  ''2 diárias com 1 grátis'', ''Reserve 2 noites e ganhe a terceira cortesia.'', ''https://images.unsplash.com/photo-1551882547-ff40c63fe5fa?auto=format&fit=crop&w=1200&q=80'', ');
INSERT INTO _seed_sql_chunks (part, content) VALUES (6, '''3x2'',
  NOW() + INTERVAL ''0 days'',
...
COMMIT;
');
DO $seed$ DECLARE q text; BEGIN SELECT string_agg(content, '' ORDER BY part) INTO q FROM _seed_sql_chunks; EXECUTE q; END $seed$;
COMMIT;
