...
AND NOT EXISTS (
  SELEC');
INSERT INTO _seed_sql_chunks (part, content) VALUES (2, 'T 1 FROM business_hours bh
  JOIN _seed_ids tb2 ON tb2.db_id = bh.business_id AND tb2.fixture_id = ''b-cachacaria''
  WHERE bh.weekday = 6
);
...
LEFT JOIN _seed_ids tn ON tn.fixture_');
INSERT INTO _seed_sql_chunks (part, content) VALUES (3, 'id = ''n-vila-sal'' AND tn.entity_type = ''neighborhood''
WHERE tt.fixture_id = ''t-salinas'' AND tt.entity_type = ''tenant''
ON CONFLICT (tenant_id, slug) DO NOTHING;
INSERT INTO _seed_ids (fixture_id, entity_type, db_id)
...
FROM _seed_ids tb
JOIN _seed_ids tt ON tt.fixture_id = ''t-salinas'' AND tt.entity_type = ''tenant''
WHERE tb.fixture_id = ''b-hotel-serra'' AND tb.');
INSERT INTO _seed_sql_chunks (part, content) VALUES (4, 'entity_type = ''business''
AND NOT EXISTS (
  SELECT 1 FROM business_hours bh
  JOIN _seed_ids tb2 ON tb2.db_id = bh.business_id AND tb2.fixture_id = ''b-hotel-serra''
  WHERE bh.weekday = 2
...
FROM _seed_ids tb
JOIN _seed_ids tt ON tt.fixture_id = ''t-salinas'' AND tt.entity_type = ''tenant''
WHERE tb.fixture_id = ''b-pet-amigo'' AND tb.entity_type =');
INSERT INTO _seed_sql_chunks (part, content) VALUES (5, ' ''business''
AND NOT EXISTS (
  SELECT 1 FROM business_hours bh
  JOIN _seed_ids tb2 ON tb2.db_id = bh.business_id AND tb2.fixture_id = ''b-pet-amigo''
  WHERE bh.weekday = 6
...
JOIN _seed_ids tt ON tt.fixture_id = ''t-salinas'' AND tt.entity_type = ''tenant''
WHERE ');
INSERT INTO _seed_sql_chunks (part, content) VALUES (6, 'tb.fixture_id = ''b-sabor-mineiro'' AND tb.entity_type = ''business''
AND NOT EXISTS (
  SELECT 1 FROM products pr
